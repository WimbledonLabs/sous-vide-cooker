...
#  'keys',
  ...
  def __getattribute__( self, attr, python_object=1 ):
    ...
    try:
      ...
      if type(result) in [ type(None), type(True), type(1), type(1J), 
    ...
    try:
      if attr!='Class':		# avoids infinite recursion
        ...
        if type(result) in [ type(True), type(1), type(1J), type(0.1), type("") ]:
          return result;
...
class Dict( PStorBaseObject ):
  ...
  def __getitem__( self, key ):
    ...
    elif type(result) in [ type(True), type(1), type(1J), type(0.1), type(""), type([]) ]:
      return result;
    else:
    # what?
      return result;
...
#      self.__key__ = key;
  ...
  def __getitem__( self, key ):
    ...
    if type(result) in [ type(True), type(1), type(1J), type(0.1), type(""), type([]) ]:
      return result;
    else:
      result.__container__ = self;
      return result;
